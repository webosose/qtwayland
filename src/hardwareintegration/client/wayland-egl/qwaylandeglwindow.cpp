...
void QWaylandEglWindow::updateSurface(bool create)
{
    ...
    QRect rect = geometry();
    QSize sizeWithMargins = geometry().size()*devicePixelRatio() + QSize(margins.left() + margins.right(), margins.top() + margins.bottom());
...
GLuint QWaylandEglWindow::contentFBO() const
{
    ...
    if (m_resize || !m_contentFBO) {
        ...
        QSize fboSize = geometry().size() * devicePixelRatio();
